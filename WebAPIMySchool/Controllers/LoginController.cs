...
namespace WebAPIMySchool.Controllers
{
    public class LoginController : ApiController
    {
        ...
        private void userLogin(string username, string password, string schoolid, string deviceid)
        {
            ...
            {
                int i = 0;
                if (dt.Rows[i]["type"].ToString() == "4" && dt.Rows[i]["device_id"].ToString() != deviceid && dt.Rows[i]["device_id"].ToString() != "") //type 4 - student
                {
                    login.loginMessage = "Invalid device ID";
                }
                else
                {
                    if (dt.Rows[i]["device_id"].ToString() == "")  // First login by student
                    {
                        string sqlQueryInserDevice = "UPDATE login SET device_id = '" + deviceid + "' WHERE ID = '" + dt.Rows[i]["ID"].ToString() + "'";
                        objDAL.ExecuteNonQuery(sqlQueryInserDevice);
                    }
        ...
        private void getStudentDetails(string studentid)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                student.firstname = dt.Rows[i]["first_name"].ToString();
            }
            //});
            //}
        }
